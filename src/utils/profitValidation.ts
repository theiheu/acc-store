...
export function validateProfitCalculations(
  ...
  if (Math.abs(profitAnalysis.revenue.total - actualRevenue) > 0.01) {
    errors.push(
      `Revenue mismatch: Analysis shows ${profitAnalysis.revenue.total.toLocaleString('vi-VN')}₫ ` +
  ...
  profitAnalysis.revenue.byProduct.forEach(product => {
    ...
    if (Math.abs(product.revenue - actualProductRevenue) > 0.01) {
      errors.push(
        ...
        `but actual orders total ${actualProductRevenue.toLocaleString('vi-VN')}₫`
      );
    }
  });
  ...
  completedOrders.forEach(order => {
    const product = dataStore.getProducts().find(p => p.id === order.productId);
    if (product) {
      let unitCost = 0;
  ...
  const profitMargin = actualRevenue > 0 ? (calculatedProfit / actualRevenue) * 100 : 0;

  // Check if profit calculation is reasonable
  if (Math.abs(profitAnalysis.profit.net - calculatedProfit) > calculatedCosts * 0.1) {
    warnings.push(
...
export function logValidationResults(result: ProfitValidationResult): void {
  ...
  console.log(`Total Revenue: ${result.summary.totalRevenue.toLocaleString('vi-VN')}₫`);
  console.log(`Calculated Profit: ${result.summary.calculatedProfit.toLocaleString('vi-VN')}₫`);
